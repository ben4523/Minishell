...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2017/03/01 12:52:35 by ybitton           #+#    #+#             */
/*   Updated: 2017/03/14 14:53:12 by ybitton          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char			*ldtoa(t_var *var, long double ld, int preci)
{
	...
	{
		if (!(str = (char*)malloc(sizeof(char) * (env.nbdiv + 7))))
			return (NULL);
		ft_memset(str, '\0', env.nbdiv + 7);
		ft_memset(str, '0', env.nbdiv + 6);
		str[env.nbdiv - 1] = '.';
		env.ret = ft_strjoin_free(env.ret, str, 2);
	}
	ft_round_off(&env);
	return (env.ret);
}
